...
| Path | What it is |
|---|---|
| `index.html` | Homepage shell for the July 2026 redesign, section by section: the hero film (`scripts/intro-film.js`) + Odometer (`scripts/home.js`), Threads of the Trail (`threads.js`), The Observatory (`observatory.js`), The Record Books (`records.js`). A small inline sessionStorage/reduced-motion check stays inline by design (must run before first paint — it fast-forwards the hero film for repeat visits and reduced-motion visitors). **Leaflet is NOT loaded here** (July 2026): the old SVG film used it for projection only, the film is MapLibre now, and `trail-renderer.js` went with it — no homepage section calls either. |
...
| `crew-member.html` + `scripts/crew-member.js` | **THE SERVICE RECORD** (`?name=Will%20R.`) — the same volume opened at one person's entry, reached by turning the page. Left leaf the record (portrait in the cover's own mount, a service block set as a ledger, and their roll lane **enlarged**); right leaf the country walked together as **numbered atlas plates** on the real Atlas basemap (Voyager + hillshade multiply + parchment wash, the same stack `map.js` wears) in year ink. **The enlarged lane is the plates' index, not decoration**: hover a mark and its trail lights on whichever plate holds it, and back the other way. Marks **stack when they collide** (measured in screen px, recomputed on resize) — two outings a day apart drew on top of each other, and a buried mark cannot be pointed at, which would make some trails unreachable on the one leaf whose job is to reach them. The 75 km complete-linkage plate clustering is kept wholesale from the old page; outings too far from the rest print as line art on a **loose sheet**. Plates never pan or zoom — roaming belongs to map.html. **Every companion has a record now**, so every companion name anywhere in the Atlas leads to the same place. Both crew pages share `styles/crew.css`. |
...
| `styles/` | `base.css` (the earthy palette as CSS custom properties + nav + footer + **the Atlas doors**: the shared `.atlas-door` component behind *every* crossing between the map, a hike page and a trip page — see "The doors" below. One brass-on-cream frame, a glyph naming the DESTINATION (open field log / folded map / surveyed section line) and a chevron-through-a-doorway pointing the way; `.is-return` mirrors both for the trip back. Only sizes are set per page — never restyle one door alone. **There is no global `box-sizing: border-box` in this codebase**, so never give a padded door `width: 100%`; `display:flex` already fills its container, and the width would push it past its neighbour) and one sheet per page or section: `home.css` (hero film + Odometer), `threads.css`, `observatory.css`, `records.css`, `map.css`, `hike.css`, `echoes.css`, `crew.css`, `trip.css`, `credits.css`. No inline CSS anywhere except `404.html` (self-contained by design). |
...
| **Record** → day | `hike.html?id=` | every mark on the enlarged lane, every trail on a plate, every trailprint on the loose sheet |
| **Record** → chapter | `trip.html?tag=` | the service block's Trips rows |
| **Roll** → day / chapter | `hike.html?id=` / `trip.html?tag=` | inside an opened lane's drawer |
| **anywhere** → record | `crew-member.html?name=` | a companion's name on a hike page, and the Runyon roll-call chips on Echoes — **every companion has a record**, so a name never leads to a generic front door. The one exception is a Runyon regular with no `hikes.json` entry at all (the Local Loop is deliberately outside it), who still gets `crew.html` rather than an unsigned page |
